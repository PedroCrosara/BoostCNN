...
def create_model(model_name, in_channels, num_classes, feature_extract, use_pretrained=True):
    ...
    elif model_name == "vit_s_16_timm":
        model_ft = timm.create_model('vit_small_patch16_224.augreg_in21k_ft_in1k',
                                     pretrained=True)
        set_parameter_requires_grad(model_ft, feature_extract)
...
class ClassificationModel(pl.LightningModule):
    ...
    def shared_epoch_end(self, outputs, stage):
        ...
        model_output = torch.cat([x["preds"] for x in outputs]).cpu()
